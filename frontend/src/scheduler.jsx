...
export function ScheduleQuickForm() {
  ...
  return <div className="quick-schedule"><h3>월간 스케줄러</h3><details open className="schedule-month"><summary>{currentMonth.replace('-', '.')}<span>{currentEntries.filter(item => !item.completed).length}</span></summary>{currentEntries.length ? tasks(currentEntries) : null}</details>{previewGroups.length > 0 && <details className="schedule-preview"><summary>미리보기 <span>{previewPendingCount}</span></summary><div className="schedule-preview-list">{previewGroups.map(([month, entries]) => <details className="schedule-preview-month" key={month}><summary>{month.replace('-', '.')}<span>{entries.filter(item => !item.completed).length}</span></summary>{tasks(entries)}</details>)}</div></details>}</div>
}
